...
import com.zionhuang.music.utils.OkHttpDownloader.DownloadException
...
object YouTubeChannelExtractor {
    suspend fun extractChannel(id: String): YouTubeChannel = withContext(Default) {
        ...
        val response = try {
            OkHttpDownloader.downloadJson(url, mapOf(
                    "content-type" to "application/json",
                    "X-YouTube-Client-Name" to "1",
                    "X-YouTube-Client-Version" to "2.20201021.03.00"
            ))
        } catch (e: DownloadException) {
            return@withContext YouTubeChannel.Error(e.message)
        }

        val initialData = response[1]["response"].asJsonObjectOrNull
