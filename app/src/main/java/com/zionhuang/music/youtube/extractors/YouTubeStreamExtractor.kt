...
class YouTubeStreamExtractor private constructor(private val context: Context) {
    ...
    suspend fun extract(videoId: String): YouTubeStream {
        val res: YouTubeStream
    ...
    private suspend fun realExtract(videoId: String): YouTubeStream = withContext(Default) extraction@{
        debug = { msg ->
        ...
        val videoWebPage = try {
            ...
            return@extraction YouTubeStream.Error(ErrorCode.NETWORK, "Unable to download the web page of the video")
        }
        ...
        if (ageGate) {
            embedWebPage = try {
                ...
                return@extraction YouTubeStream.Error(ErrorCode.NETWORK, "Unable to download the embed web page of the video")
            }
            val videoInfoWebPage = try {
                ...
                return@extraction YouTubeStream.Error(ErrorCode.NETWORK, "Unable to download the info of the video")
            }
            ...
            if (ytPlayerConfig != null) {
                ...
                if (videoInfo == null && "ypc_vid" in args) {
                    debug("Rental video")
                    return@extraction YouTubeStream.Error(ErrorCode.RENTAL, "Rental video not supported")
                }
        ...
        if ("ypc_video_rental_bar_text" in videoInfo && "author" !in videoInfo) {
            YouTubeStream.Error(ErrorCode.RENTAL, "Rental video not supported")
        }
        ...
        if (videoInfo["conn"].asStringOrNull?.startsWith("rtmp") == true) {
            ...
            if ("rtmpe%3Dyes" in "${videoInfo["url_encoded_fmt_stream_map"][0].asStringOrBlank},${videoInfo["adaptive_fmts"][0].asStringOrBlank}") {
                YouTubeStream.Error(ErrorCode.RTMPE, "rtmpe downloads are not supported")
            }
        ...
        if (formats.isEmpty() && "license_info" in videoInfo || "licenseInfos" in playerResponse["streamingData"].asJsonObjectOrNull) {
            YouTubeStream.Error(ErrorCode.DRM, "The video is drm protected")
        }

        YouTubeStream.Success(
                id = videoId,
                title = videoTitle,
                channelId = channelId,
    ...
    companion object {
        @Volatile
        private var INSTANCE: YouTubeStreamExtractor? = null

        @JvmStatic
        fun getInstance(context: Context): YouTubeStreamExtractor {
            if (INSTANCE == null) {
                synchronized(YouTubeStreamExtractor::class.java) {
                    if (INSTANCE == null) {
                        INSTANCE = YouTubeStreamExtractor(context)
                    }
                }
            }
            return INSTANCE!!
