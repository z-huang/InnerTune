...
class YouTubeTest {
    ...
    @Test
    fun `Check search continuation`() = try {
        runBlocking {
            var count = 5
    ...
    @Test
    fun `Check 'get_search_suggestion' endpoint`() = try {
        runBlocking {
    ...
    @Test
    fun `Check 'browse' continuation`() = try {
        runBlocking {
    ...
    @Test
    fun `Check 'next' continuation`() = try {
        runBlocking {
            val videoId = "qivRUhepWVA"
            val playlistId = "RDEMQWAKLFUHzBCn9nEsPHDYAw"
            var count = 5
    ...
    @Test
    fun `Check 'get_queue' endpoint`() = try {
        runBlocking {
